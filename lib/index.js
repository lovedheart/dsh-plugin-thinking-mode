  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        ...
        "The optional 'preserveThinking' parameter (boolean) sets whether prior turns' reasoning content is kept in the " +
        "context (chat_template_kwargs.preserve_thinking); it only has an effect while thinking is on (while off it is sent as false). " +
        "Convention: when the user wants to enable thinking, first ASK the user (via ask_user_question) whether to preserve " +
        "the reasoning content, then pass their answer as 'preserveThinking' in this call. When disabling thinking, " +
        "preserveThinking is automatically false — no need to ask. " +
      ...
      parameters: {
        ...
        preserveThinking: {
          type: "boolean",
          description:
            "Optional: set whether prior turns' reasoning content is kept in the context " +
            "(chat_template_kwargs.preserve_thinking). Only takes effect while thinking is on — while " +
            "thinking is off the request always sends preserve_thinking: false. Convention: when the user " +
            "wants to enable thinking, first ask the user (ask_user_question) whether to preserve the " +
            "reasoning content, then pass their answer here. Omit to leave the current choice unchanged.",
        },
      },
      output: {
        schema: {
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      ...
      return (
        ...
        `Current preserve_thinking: ${preserve} (whether prior turns' reasoning content is kept in the context; ` +
        `only effective while thinking is on, always sent as false while off). ` +
        ...
        `When the user wants to ENABLE thinking, first ask the user via ask_user_question whether to preserve the ` +
        `reasoning content in the context, then pass their answer as the "preserveThinking" parameter in the same ` +
        `thinking_mode call; when DISABLED, preserveThinking is automatically false and you do not need to ask.`
      );
    },
  });
