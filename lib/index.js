  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      ...
      return (
        ...
        `To change reasoning depth, update the "reasoningEffort" field of the thinking-mode settings section ` +
        `(or say so in conversation so the assistant can do it via the settings service); it is applied to ` +
        `model calls from the next turn onward and only affects thinking mode.`
      );
    },
  });
