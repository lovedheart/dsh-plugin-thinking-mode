...
import { SAMPLING, presetJsonSchema, presetSchemastery } from "./messages.js";
          ...
          if (value.interceptActive === true) {
            parts.push(
              " The intercept is ACTIVE for the default model route (requests are routed with the matching enable_thinking setting" +
                (value.sampling?.applySampling
                  ? ` and the mode's recommended sampling: temperature=${value.sampling[value.mode === "on" ? "thinking" : "instruct"]?.temperature}, top_p=${value.sampling[value.mode === "on" ? "thinking" : "instruct"]?.top_p}, presence_penalty=${value.sampling[value.mode === "on" ? "thinking" : "instruct"]?.presence_penalty}`
                  : "") +
                ").",
            );
