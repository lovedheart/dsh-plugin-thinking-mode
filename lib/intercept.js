 ...
 *   undefined when sampling pass-through is requested), the row config, and
 *   the named logger.
 */
export function installThinkingModeIntercept(ctx, { getMode, getSampling, config, log }) {
  ...
  async function* directStream(options, target, mode, attachments) {
    ...
    try {
      ...
      const body = buildBody(options, thinkingOn, wireMessages, { sampling });
